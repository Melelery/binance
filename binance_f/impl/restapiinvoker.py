...
def call_sync(request):
    if request.method == "GET":
        # response = requests.get(request.host + request.url, headers=request.header,timeout=(1,1))
        response =  json.loads(REQUESTS_SESSION.get(request.host + request.url, headers=request.header,timeout=(1,1)).content.decode())
        return response
    elif request.method == "POST":
        response =  json.loads(REQUESTS_SESSION.post(request.host + request.url, headers=request.header,timeout=(1,1)).content.decode())
        # response = requests.post(request.host + request.url, headers=request.header,timeout=(1,1))
        return response
    elif request.method == "DELETE":
        response =  json.loads(REQUESTS_SESSION.delete(request.host + request.url, headers=request.header,timeout=(1,1)).content.decode())
        # response = requests.post(request.host + request.url, headers=request.header,timeout=(1,1))
        return response
    elif request.method == "PUT":
        response =  json.loads(REQUESTS_SESSION.put(request.host + request.url, headers=request.header,timeout=(1,1)).content.decode())
        # response = requests.post(request.host + request.url, headers=request.header,timeout=(1,1))
        return response
