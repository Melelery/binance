...
class RestApiRequestImpl(object):
    ...
    def post_market_order(self, symbol, side, ordertype, 
        ...
        if not reduceOnly:
            reduceOnlyStr = "false"
        extraUrl = "newClientOrderId="+newClientOrderId+"&symbol="+symbol+"&side="+side+"&type="+ordertype+"&quantity="+str(quantity)+"&reduceOnly="+reduceOnlyStr+"&positionSide="+positionSide+"&recvWindow=60000&timestamp="+ts+""
    ...
    def get_order_by_system_id(self, symbol, systemOrderId):
        request = RestApiRequest()
        request.method = "GET"
        request.host = self.__server_url
        ts = str(get_current_timestamp() -1000)
        extraUrl = "symbol="+symbol+"&orderId="+systemOrderId+"&recvWindow=60000&timestamp="+ts+""
    ...
    def change_position_side(self):
        request = RestApiRequest()
        request.method = "POST"
        request.host = self.__server_url
        ts = str(get_current_timestamp() -1000)
        extraUrl = "dualSidePosition=false&"+"recvWindow=60000&timestamp="+ts+""

        sign=create_signature(self.__secret_key, extraUrl)
        extraUrl = extraUrl+"&signature="+sign
        request.header.update({"Content-Type": "application/x-www-form-urlencoded"})
        request.header.update({"X-MBX-APIKEY": self.__api_key})
        request.url =  "/fapi/v1/positionSide/dual?"+extraUrl
