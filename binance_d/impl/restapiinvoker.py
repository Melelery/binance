...
def call_sync(request):
    if request.method == "GET":
        ...
        response = requests.delete(request.host + request.url, headers=request.header,timeout=(1,1))
        return response.text
    elif request.method == "PUT":
        response = requests.put(request.host + request.url, headers=request.header,timeout=(1,1))
        return response.text
