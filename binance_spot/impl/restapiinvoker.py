import requests
from binance_spot.exception.binanceapiexception import BinanceApiException
from binance_spot.impl.utils import *
...
def call_sync(request):
    if request.method == "GET":
        response = requests.get(request.host + request.url, headers=request.header,timeout=(3,3))
        return response.text
    elif request.method == "POST":
        response = requests.post(request.host + request.url, headers=request.header,timeout=(3,3))
        return response.text
    elif request.method == "DELETE":
        response = requests.delete(request.host + request.url, headers=request.header,timeout=(3,3))
        limits = get_limits_usage(response)
        json_wrapper = parse_json_from_string(response.text)
        check_response(json_wrapper)
        return response.text
    elif request.method == "PUT":
        response = requests.put(request.host + request.url, headers=request.header,timeout=(3,3))
        limits = get_limits_usage(response)
        json_wrapper = parse_json_from_string(response.text)
        check_response(json_wrapper)
