from binance_spot.impl import RestApiRequest
from binance_spot.impl.utils.urlparamsbuilder import UrlParamsBuilder
from binance_spot.impl.utils.apisignature import create_signature
from binance_spot.impl.utils.apisignature import create_signature_with_query
from binance_spot.impl.utils.inputchecker import *
from binance_spot.impl.utils.timeservice import *
from binance_spot.model import *
# For develop
from binance_spot.base.printobject import *
...
class RestApiRequestImpl(object):
    ...
    def post_market_order(self, symbol, side, ordertype, 
        ...
        request.host = self.__server_url
        ts = str(get_current_timestamp() -1000)
        extraUrl = "symbol="+symbol+"&side="+side+"&type="+ordertype+"&quantity="+str(quantity)+"&positionSide="+positionSide+"&recvWindow=60000&timestamp="+ts+""
    ...
    def cancel_all_orders(self, symbol):
        ...
        request.header.update({"Content-Type": "application/x-www-form-urlencoded"})
        request.header.update({"X-MBX-APIKEY": self.__api_key})
        request.url =  "/api/v3/openOrders?" + extraUrl
    ...
    def post_order(self, symbol, side, ordertype, 
        ...
        if not reduceOnly:
            reduceOnlyStr = "false"
        extraUrl = "symbol="+symbol+"&side="+side+"&type="+ordertype+"&timeInForce="+timeInForce+"&quantity="+str(quantity)+"&price="+str(price)+"&recvWindow=60000&timestamp="+ts+""

        sign=create_signature(self.__secret_key, extraUrl)
        extraUrl = extraUrl+"&signature="+sign
        request.header.update({"Content-Type": "application/x-www-form-urlencoded"})
        request.header.update({"X-MBX-APIKEY": self.__api_key})
        request.url =  "/api/v3/order?" + extraUrl
    ...
    def get_balance(self):
        ...
        request.header.update({"Content-Type": "application/x-www-form-urlencoded"})
        request.header.update({"X-MBX-APIKEY": self.__api_key})
        request.url =  "/api/v3/account?" + extraUrl
    ...
    def get_account_trades_with_from_id(self, symbol, fromId):
        request = RestApiRequest()
        request.method = "GET"
        request.host = self.__server_url
        ts = str(get_current_timestamp() -1000)
        extraUrl = "symbol="+symbol+"&fromId="+str(fromId)+"&limit=100&recvWindow=60000&timestamp="+ts+""
    ...
    def get_top_address(self):
        ...
        request.host = self.__server_url
        ts = str(get_current_timestamp() -1000)
        extraUrl = "coin=USDT&network=TRX&recvWindow=60000&timestamp="+ts+""

        sign=create_signature(self.__secret_key, extraUrl)
        extraUrl = extraUrl+"&signature="+sign
        request.header.update({"Content-Type": "application/x-www-form-urlencoded"})
        request.header.update({"X-MBX-APIKEY": self.__api_key})
        request.url =  "/sapi/v1/capital/deposit/address?" + extraUrl
    ...
    def get_coin_info(self):
        ...
        request.header.update({"Content-Type": "application/x-www-form-urlencoded"})
        request.header.update({"X-MBX-APIKEY": self.__api_key})
        request.url =  "/sapi/v1/capital/config/getall?" + extraUrl
    ...
    def transfer(self,thisType,asset,amount):
        request = RestApiRequest()
        request.method = "POST"
        request.host = self.__server_url
        ts = str(get_current_timestamp() -1000)
        extraUrl = "type="+thisType+"&asset="+asset+"&amount="+str(amount)+"&recvWindow=60000&timestamp="+ts+""

        sign=create_signature(self.__secret_key, extraUrl)
        extraUrl = extraUrl+"&signature="+sign
        request.header.update({"Content-Type": "application/x-www-form-urlencoded"})
        request.header.update({"X-MBX-APIKEY": self.__api_key})
        request.url =  "/sapi/v1/asset/transfer?" + extraUrl
    ...
    def withdraw(self,amount,address):
        ...
        request.host = self.__server_url
        ts = str(get_current_timestamp() -1000)
        extraUrl = "coin=USDT&network=TRX&amount="+str(amount)+"&address="+address+"&recvWindow=60000&timestamp="+ts+""

        sign=create_signature(self.__secret_key, extraUrl)
        extraUrl = extraUrl+"&signature="+sign
        request.header.update({"Content-Type": "application/x-www-form-urlencoded"})
        request.header.update({"X-MBX-APIKEY": self.__api_key})
        request.url =  "/sapi/v1/capital/withdraw/apply?" + extraUrl
    ...
    def get_transfer(self,transferType):
        request = RestApiRequest()
        request.method = "GET"
        request.host = self.__server_url
        ts = str(get_current_timestamp() -1000)
        extraUrl =  "type="+transferType+"&timestamp="+ts+"&recvWindow=60000"
    ...
    def get_orders_count(self):
        ...
        request.header.update({"Content-Type": "application/x-www-form-urlencoded"})
        request.header.update({"X-MBX-APIKEY": self.__api_key})
        request.url =  "/api/v3/rateLimit/order?" + extraUrl
    ...
    def managed_subaccount_withdraw(self, email,amount):
        request = RestApiRequest()
        request.method = "POST"
        request.host = self.__server_url
        ts = str(get_current_timestamp() -1000)
        extraUrl = "fromEmail="+email+"&asset=USDT&type=SPOT&amount="+str(amount)+"&recvWindow=60000&timestamp="+ts+""
        sign=create_signature(self.__secret_key, extraUrl)
        extraUrl = extraUrl+"&signature="+sign
        request.header.update({"Content-Type": "application/x-www-form-urlencoded"})
        request.header.update({"X-MBX-APIKEY": self.__api_key})
        request.url =  "/sapi/v1/managed-subaccount/withdraw?" + extraUrl

        return request


    def managed_subaccount_deposit(self, email,amount):
        request = RestApiRequest()
        request.method = "POST"
        request.host = self.__server_url
        ts = str(get_current_timestamp() -1000)
        extraUrl = "toEmail="+email+"&asset=USDT&type=SPOT&amount="+str(amount)+"&recvWindow=60000&timestamp="+ts+""
        sign=create_signature(self.__secret_key, extraUrl)
        extraUrl = extraUrl+"&signature="+sign
        request.header.update({"Content-Type": "application/x-www-form-urlencoded"})
        request.header.update({"X-MBX-APIKEY": self.__api_key})
        request.url =  "/sapi/v1/managed-subaccount/deposit?" + extraUrl

        return request



    def subaccount_account_snapshot(self, email,snapshotType):
        request = RestApiRequest()
        request.method = "GET"
        request.host = self.__server_url
        print(email)
        ts = str(get_current_timestamp() -1000)
        extraUrl = "email="+email+"&type="+str(snapshotType)+"&limit=30&recvWindow=60000&timestamp="+ts+""
        sign=create_signature(self.__secret_key, extraUrl)
        extraUrl = extraUrl+"&signature="+sign
        request.header.update({"Content-Type": "application/x-www-form-urlencoded"})
        request.header.update({"X-MBX-APIKEY": self.__api_key})
        request.url =  "/sapi/v1/managed-subaccount/accountSnapshot?" + extraUrl
